    ...
    return function ({ addComponents, addBase, matchUtilities, theme }) {
        const config = {
            ...
            components: {
                ...
                popover: setPopoverConfig(options.components.popover),
            },
        };
        addBase({
            ":root": {
                ...
                "--alert-shadow":
                    getShadows("alert")[config.components.alert.shadow.size ?? config.shadow.size],
                ...
                "--card-border-radius": theme(
                    `borderRadius.${config.components.card.rounded ?? radius[options.radius].card}`
                ),
                "--card-shadow":
                    getShadows("card")[config.components.card.shadow.size ?? config.shadow.size],
                "--card-shadow-opacity": `${
                ...
                "--card-dark-bg": theme(
                    `colors.gray.${
                        config.components.card.dark.bg ?? background[options.background].dark.card
                    }`
                ),

                // Menu
                "--menu-border-radius": theme(
                    `borderRadius.${config.components.menu.rounded ?? radius[options.radius].menu}`
                ),
                "--menu-light-bg-opacity": `${config.components.menu.bgOpacity ?? 90}%`,
                "--menu-dark-bg-opacity": `${config.components.menu.dark.bgOpacity ?? 50}%`,
                "--menu-dark-bg": theme(
                    `colors.gray.${
                        config.components.menu.dark.bg ?? background[options.background].dark.menu
                    } / var(--menu-dark-bg-opacity)`
                ),
                "--menu-shadow":
                    getShadows("menu")[config.components.menu.shadow.size ?? config.shadow.size],
                "--menu-shadow-opacity": `${
                ...
                "--flag-border-radius": theme(
                    `borderRadius.${config.components.flag.rounded ?? radius[options.radius].flag}`
                ),
                "--flag-padding": theme(
                ...
                "--popover-border-radius": theme(
                    `borderRadius.${
                        config.components.popover.rounded ?? radius[options.radius].popover
                    }`
                ),
                ...
                // Switch
                "--switch-border-radius": theme(`borderRadius.${radius[options.radius].switch}`),
                ...
                // Tooltip
                "--tooltip-border-radius": theme(`borderRadius.${radius[options.radius].tooltip}`),

                // Toast
                "--toast-border-radius": theme(`borderRadius.${radius[options.radius].toast}`),

                // Separator
                "--separator-light-color": theme(`colors.gray.${borderColors[config.border].ui}`),
                "--separator-dark-color": theme(
                    `colors.gray.${borderColors[config.border].dark.ui}`
                ),

                // UI
                "--ui-light-border-color": theme(`colors.gray.${borderColors[config.border].ui}`),
