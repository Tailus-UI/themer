...
const baseTrigger =
    "relative h-full flex-1 flex gap-2 items-center justify-center leading-none select-none rounded-[--tabs-soft-border-radius] cursor-pointer";
...
const getTriggerColor = (color, colors = softTabsColors) => {
    return baseTrigger + " " + colors[color];
};
